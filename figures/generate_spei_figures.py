...
def generate_fit_heatmap(window: int = 1):
    ...
    try:
        spei = SPEI()
        fit_check = spei.check_fit(window=window)
        ...
        print(f"Figure spei/fit/spei_fit_{window}.svg skipped.")
...
def generate_series_plot(window: int = 1):
    ...
    try:
        ...
        for i, area in enumerate(["Bitterroot", "Gallatin"]):
            ...
            ax[i].set_ylim(
                np.min([np.min(temp_df["SPEI"]), -3]),
                np.max([np.max(temp_df["SPEI"]), 3]),
            )
        ...
        fig.suptitle(f"SPEI-{window}", fontweight="bold")
        fig.tight_layout()
        fig.savefig(f"spei/series/spei_{window}.svg", dpi=300, format="svg")
        plt.close(fig)
        print(f"Figure spei/series/spei_series_{window}.svg generated.")
    except FileNotFoundError:
        print(f"Figure spei/series/spi_series_{window}.svg skipped.")
